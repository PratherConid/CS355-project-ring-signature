...
fn pow_65537(
    ...
    let mut value_pow = value.clone();
    for _ in 0..16 {
        value_pow = builder.mul_biguint(&value_pow, &value_pow);
        value_pow = builder.rem_biguint(&value_pow, modulus);
    }
    value_pow = builder.mul_biguint(&value_pow, value);
    builder.rem_biguint(&value_pow, &modulus)
}
...
pub fn compute_padded_hash(message_hash: &BigUint) -> BigUint {
    ...
    if RSA_MODULUS_BYTES <= HASH_BYTES + 3 {
        panic!("RSA_MODULUS_BYTES <= HASH_BYTES + 3");
    }
...
pub fn create_ring_circuit(max_num_pks: usize) -> RingSignatureCircuit {
    ...
    // Add additional targets for the signature and public keys
    ...
    for i in 0..max_num_pks {
        let pk_eq = builder.eq_biguint(&pk_targets[i], &sig_pk_target);
        pk_in_set = builder.or(pk_in_set, pk_eq);
    }
    let one = builder.one();
    builder.connect(vrfy.target, one);
    builder.connect(pk_in_set.target, one);
...
pub fn create_ring_proof(
    ...
    for (value, target) in public_keys.iter().zip(circuit.pk_targets.iter()) {
        pw.set_biguint_target(target, &value.n)?;
    }
